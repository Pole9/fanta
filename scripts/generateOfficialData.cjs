...
const fs = require('fs');
const { getPrevious3Seasons } = require('./previousClubData.cjs');
...
const formatSeason = (seasonLabel, raw, role) => {
  if (!raw) return null;
  const pg = Number(raw.Pv) || 0;
  // Titolare: portiere con almeno 18 presenze, giocatore di movimento con almeno 20 presenze a voto
  const isTitolare = (role === 'P' && pg >= 18) || (role !== 'P' && pg >= 20);

  return {
    season: seasonLabel,
    squadra: String(raw.Squadra || '').trim(),
    isTitolare: Boolean(isTitolare),
    pg,
...
const players = listoneRaw.map(p => {
  ...
  const st25 = formatSeason('2025/26', map2526.get(id), role);
  const st24 = formatSeason('2024/25', map2425.get(id), role);
  const st23 = formatSeason('2023/24', map2324.get(id), role);

  const validSeasons = [st25, st24, st23].filter(Boolean);
  const isPrimoAnnoA = validSeasons.length === 0;
  let squadraPrecedente = '';
  const seasonsMap = {
    '2025/26': st25,
    '2024/25': st24,
    '2023/24': st23
  ...
  if (isPrimoAnnoA) {
    const prevInfo = getPrevious3Seasons(id, String(p.Squadra || '').trim(), role);
    squadraPrecedente = prevInfo.squadraPrecedente;
    if (Array.isArray(prevInfo.seasons)) {
      prevInfo.seasons.forEach(s => {
        validSeasons.push(s);
        seasonsMap[s.season] = s;
      });
    }
  }

  const latest = validSeasons[0] || null;
  ...
  // Centrocampisti offensivi (Trequartisti, Tornanti, Attaccanti esterni: T, W, A, E)
  const rawMantra = String(p.RM || p.Rm || '').trim().toUpperCase();
  const isTq = role === 'C' && (
    rawMantra.includes('T') || 
    rawMantra.includes('W') || 
    rawMantra.includes('A') || 
    rawMantra.includes('E')
  );
  ...
  return {
    ...
    status: 'available',
    isPrimoAnnoA,
    squadraPrecedente: isPrimoAnnoA ? squadraPrecedente : undefined,
    seasons: seasonsMap,
    threeYearAvg: {
      mv: threeYearMvAvg,
      fm: threeYearFmAvg,
...
console.log('Successfully regenerated src/data/officialPlayers.ts with squadra and isTitolare.');
